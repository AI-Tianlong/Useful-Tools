...
def convert_bbox_info(label, idx, obj_count, image_height, image_width):
    ...
    class_name = {'ZhanDouJi': 0, 'YunShuJi': 1, 'BaJi': 2}
    cls_id = class_name[label[5]]
...
def organize_by_existing_files(image_dir: str, existed_categories: list):
    ...
    for cat in categories:
        if cat in existed_categories:
            txt_file = osp.join(image_dir, f'{cat}.csv')
...
def convert_yolo_to_coco(image_dir: str):
    ...
    for idx, image in enumerate(mmengine.track_iter_progress(indices)):
        ...
        label_path = f'{osp.join(yolo_label_dir, img_name)}.csv'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--image_dir',
        type=str,
        default='/media/atl/Data1/ATL/AI_work/ISAR/Dataset/ATL_ISAR/train',
        help='dataset directory with ./images and ./labels, classes.txt, etc.')
    arg = parser.parse_args()
    # convert_yolo_to_coco(arg.image_dir)
    convert_yolo_to_coco('/media/atl/Data1/ATL/AI_work/ISAR/Dataset/ATL_ISAR/train')
    convert_yolo_to_coco('/media/atl/Data1/ATL/AI_work/ISAR/Dataset/ATL_ISAR/val')
